...
Winwheel.prototype.clearCanvas = function() { this.ctx && this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height) };
...
Winwheel.prototype.degToRad = function(a) { return .017453292519943295 * a };
...
Winwheel.prototype.windowToCanvas = function(a, b) { var c = this.canvas.getBoundingClientRect(); return { x: Math.floor(a - this.canvas.width / c.width * c.left), y: Math.floor(b - this.canvas.height / c.height * c.top) } };
...
Winwheel.prototype.getSegmentNumberAt = function(a, b) {
    ...
    for (a = 1; a <= this.numSegments; a++)
        if (c >= this.segments[a].startAngle && c <= this.segments[a].endAngle && n >= this.innerRadius && n <= this.outerRadius) { e = a; break }
    return e
};
...
Winwheel.prototype.getIndicatedSegmentNumber = function() {
    ...
    for (x = 1; x < this.segments.length; x++)
        if (b >= this.segments[x].startAngle && b <= this.segments[x].endAngle) { a = x; break }
    return a
};
...
Winwheel.prototype.pauseAnimation = function() { this.tween && this.tween.pause() };
Winwheel.prototype.resumeAnimation = function() { this.tween && this.tween.play() };
...
function Animation(a) {
    defaultOptions = { type: "spinOngoing", direction: "clockwise", propertyName: null, propertyValue: null, duration: 10, yoyo: !1, repeat: 0, easing: "power3.easeOut", stopAngle: null, spins: null, clearTheCanvas: null, callbackFinished: null, callbackBefore: null, callbackAfter: null };
    for (var b in defaultOptions) this[b] = null != a && "undefined" !== typeof a[b] ? a[b] : defaultOptions[b];
    if (null != a)
        for (b in a) "undefined" === typeof this[b] && (this[b] = a[b])
}
...
function PointerGuide(a) { defaultOptions = { display: !1, strokeStyle: "red", lineWidth: 3 }; for (var b in defaultOptions) this[b] = null != a && "undefined" !== typeof a[b] ? a[b] : defaultOptions[b] }
...
function winwheelStopAnimation(a) { 0 != a && null != winwheelToDrawDuringAnimation.animation.callbackFinished && eval(winwheelToDrawDuringAnimation.animation.callbackFinished) }
